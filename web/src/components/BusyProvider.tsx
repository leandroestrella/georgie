...
interface BusyContextValue {
  /** True while at least one admin write is in flight. */
  busy: boolean
...
/**
 * Tracks how many admin writes are in flight app-wide, driving a single
 * loading overlay (see `LoadingOverlay`) instead of each write control
 * managing its own visual feedback. A ref-counted counter rather than a
 * plain boolean, since a page can trigger more than one sequential write
 * (e.g. confirming an exchange also updates the linked book).
 */
